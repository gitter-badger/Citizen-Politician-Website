...
class Accounts extends CI_Model {
	...
	public function login_politician($user_name){
		return $this->db->query("select userName as username,Password as pass,photo,accountType as usertype from politician_profile where (userName=? or email=? or phone=?) and accountVerified=1",array($user_name,$user_name,$user_name))->row();
	}
	...
	//since email,phone and username are unique fields in the database, this function gets the email,username and phone number for any user in the system. One can pass an email, username or phone number as a parameter to this function. Thus it can be used to check if the email, username or phone number exists in the database. It can also be used to get the other parameters given one of them.
	...
	//Incase a politician is denied access to the site on login, this function checks why they were denied access ie what credentials were missing eg account is non existent or account is not verified.
	public function check_login_error($politician){
		$error=$this->db->query("select userName as username,Password as pass,photo,accountType as usertype,countyNo as county from politician_profile where (userName=? or email=? or phone=?) and accountVerified=0",array($politician,$politician,$politician))->row();
		if(!isset($error)){
			return "not found";
		}
		return "account";
	...
	public function add_citizen($username,$email,$email_verified,$phone,$phone_verified,$gender,$dob,$country,$county,$secret){
		...
		return $this->db->query("insert into citizen_profile (UserName,Email,verifyEmail,phone,verifyPhone,gender,DOB,country,County,photo,Secret) values (?,?,?,?,?,?,?,?,?,?,?)",array(strtolower(trim($username)),strtolower(trim($email)),$email_verified,$phone,$phone_verified,strtolower(trim($gender)),trim($dob),trim($country),trim($county),trim($photo),password_hash($secret,PASSWORD_DEFAULT)));
	}
