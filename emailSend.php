	...
	if($stmt){
		...
		if($row=$stmt->fetch_array(MYSQLI_NUM)){
			...
			if($type==="password"||$type==="email"||$type==="phone"){
				$stmt=$connection->query("update emailGetCredentials set used=1 where userEmail='$user' and type='$type'");
				...
				if($stmt){
					...
					if($stmt->bind_param("isss",$newID,$user,$passCode,$type)){
						if($stmt->execute()){
							if($type==="password"){
								$_POST['subject']="Password Reset Request.";
								$_POST['message']="We hope that you are well. Below is a link that will enable you to reset your password. <br><br> <a style='text-align:center;border-radius:5px;background-color: #03869b;color: white;padding:10px;' href='https://mwananchi.herokuapp.com/ResetPassword.php?id=".password_hash($user,PASSWORD_DEFAULT)."&encodedPassCode=$passCode'>Reset My Password</a><br>";
							}
						}
					}
				}else{
